...
func CCC(ctx context.Context) error {
	...
	if err := i.PostImage(ctx, slackChannel, buf, fmt.Sprintf("%s.%s.%s.%s.%s", projectID, datasetName, tableName, to.Format(constz.DateOnly), imageFormat), fmt.Sprintf("%s %s", projectID, to.Format(constz.DateOnly))); err != nil {
